...
import { InputGroup, InputGroupAddon, InputGroupButton, InputGroupInput } from "@/features/shadcn/ui/input-group";
import { ChevronDownIcon, XIcon, CheckIcon } from "lucide-react";
import { cn } from "@/features/shadcn/lib/utils";
import { Button } from "@/features/shadcn/ui/button";
import { rippleEnable } from "@/features/shadcn/lib/ripple";
...
function ComboboxTrigger({ className, children, onPointerDown, ...props }: ComboboxPrimitive.Trigger.Props) {
	return (
		<ComboboxPrimitive.Trigger
			data-slot="combobox-trigger"
			className={cn("ripple relative [&_svg:not([class*='size-'])]:size-4", className)}
			onPointerDown={(e) => {
				onPointerDown?.(e);
				rippleEnable(e);
			}}
...
function ComboboxItem({ className, children, onPointerDown, ...props }: ComboboxPrimitive.Item.Props) {
	return (
		<ComboboxPrimitive.Item
			...
			className={cn(
				"ripple relative flex min-h-7 w-full cursor-default items-center gap-2 rounded-xl py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
				className,
			)}
			{...props}
		>
